...
export class FractalExplorer {
    static async create({ divContainer, renderingEngine, options, onMapChanged, onRendered } = {}) {
        const explorer = new FractalExplorer({ divContainer, renderingEngine, options, onMapChanged, onRendered });
        await explorer.#initRenderer();
        return explorer;
    }

    constructor({ divContainer, renderingEngine, options, onMapChanged, onRendered } = {}) {
    ...
    async #initRenderer() {
        ...
        this.renderer = await newRenderer(this.canvas, this.ctx, this.map, this.renderingEngine);
    ...
    #onMouseMove(e) {
        ...
        const oldPos = this.#canvasToComplex(this.lastMousePos.x, this.lastMousePos.y);
    ...
    #render() {
        ...
        const restPixelDensity = this.renderer.id() === RenderingEngine.WEBGPU ? 8 : 1;
        ...
        const renderContext = this.renderer.render(this.map, new RenderOptions({ pixelDensity, deep, maxIter, palette, fn }))
        this.onRendered?.(renderContext);

        clearTimeout(this.renderTimeoutId);
        if (pixelDensity !== restPixelDensity) {
            this.renderTimeoutId = setTimeout(() => {
                const renderContext = this.renderer.render(this.map, new RenderOptions({ pixelDensity: restPixelDensity, deep, maxIter, palette, fn }));
                this.onRendered?.(renderContext);
            }, 300);
        }
    }
    ...
    #canvasToComplex(sx, sy) {
        return this.map.screenToComplex(sx * DPR, sy * DPR, this.canvas.width, this.canvas.height);
    }
}
